# Changelog

All notable changes to this project will be documented in this file.

## [v0.1.10] - 2026-01-23

### ✨ Features (新特性)
*   **繁简切换**: 在歌词面板新增 **"繁"** 字开关，支持一键切换歌词为繁体中文或简体中文。
*   **数据升级**: 重构了歌词数据库，现在同时内置了简体与繁体两个版本的歌词文本，确保转换准确无误。
*   **桌面歌词同步**: 桌面歌词现在会实时跟随繁/简设置进行同步更新。

### 💄 UI/UX (体验优化)
*   **布局调整**:
    *   将“紧凑模式”勾选框移至“歌词”标签页底部，方便在编辑时快速调整。
    *   将“文字排版”面板整体归类到“样式”标签页，使界面逻辑更加清晰。
*   **细节修复**: 统一了歌词编辑区输入框的边框与背景颜色，使其完美适配所有应用主题。

## [v0.1.7] - 2026-01-17

### ✨ Features (新特性)
*   **多主题支持**:
    *   新增5款基于陈奕迅经典专辑配色的应用主题
    *   **CHIN UP!**：清爽蓝调，充满活力与正能量
    *   **DUO**：经典香槟金，温暖优雅
    *   **L.O.V.E.**：都市牛仔蓝，现代简约
    *   **FEAR&DREAMS**：现代画廊灰，深沉玫瑰红
    *   **THE KEY**：复古米色，深邃暗红
    *   主题选择器UI带颜色预览，设置自动持久化保存

### 💄 UI/UX (体验优化)
*   **CSS重构**: 使用CSS自定义属性实现动态主题切换系统
*   **界面优化**: 移除点状背景图案，界面更简洁清爽
*   **图标适配**: 所有图标颜色使用主题感知的CSS变量

## [v0.1.6] - 2026-01-16

### ✨ Features (新特性)
*   **海报工坊 2.0 (Poster Studio 2.0)**:
    *   **模块化重构**: 全新设计的控制面板，操作更直观。
    *   **多比例画布**: 支持多种画布比例（如 9:16, 1:1, 3:4 等），完美适配各类社交媒体。
    *   **图片滤镜**: 内置多款精选滤镜，一键提升配图质感。
    *   **高清导出**: 优化渲染引擎，输出更高分辨率的海报。
    *   **水印控制**: 支持自由开启/关闭 "Eason Moment" 品牌水印。
*   **体验升级**:
    *   **配置记忆**: 桌面歌词颜色等设置支持持久化保存。
    *   **智能配色**: 歌曲信息颜色会自动根据歌词颜色进行协调适配。

### 💄 UI/UX (体验优化)
*   **底层重构**: 移除了冗余的遗留代码，优化了应用体积与运行效率。

## [v0.1.5] - 2026-01-15

### ✨ Features (新特性)
*   **启动优化**: 软件启动时默认自动开启桌面歌词，无需手动开启。

### 💄 UI/UX (体验优化)
*   **macOS 托盘优化**: 为菜单栏图标启用了模板模式 (`Template Image`)，完美适配系统深色/浅色模式切换。
*   **标题显示**: 优化了 macOS 菜单栏歌词标题的截断长度，视觉上更加整洁。

### 🐞 Bug Fixes (修复)
*   **语法修复**: 修复了 `electron/main.ts` 中 `window-all-closed` 事件处理器的语法错误导致的编译失败。
*   **稳定性**: 完善了 `before-quit` 事件处理，确保程序退出时能够彻底销毁所有窗口进程。

## [v0.1.0] - 2026-01-13

### 🎉 Initial Release

**Eason Moment** 的首个公开预览版本，带来了跨平台的沉浸式歌词体验。

### ✨ Features (新特性)

*   **多平台支持**: 完美适配 macOS (Hidden Inset Titlebar) 和 Windows (Custom Frameless Window)。
*   **海报工坊**:
    *   内置 3 款主题：经典沉浸、文艺拍立得、电影故事。
    *   支持**本地图片上传**与交互式布局（拖拽、缩放）。
    *   支持导出高清 PNG，提供旋转/脉动加载特效。
    *   排版参数（字号/行距）与图片位置支持**独立重置**。
*   **智能托盘**:
    *   **macOS**: 顶部菜单栏显示歌词摘要。
    *   **心情模式**: 6 大分类（含新增的 **💕 爱与浪漫**），智能聚合 20+ 种标签。
    *   **轮播控制**: 支持自定义轮播间隔（1分钟 - 1小时）。
*   **桌面歌词 (Desktop Lyric)**:
    *   Windows 专属的透明悬浮窗。
    *   支持全窗口拖拽。
    *   支持**自定义文字颜色**，并提供系统级取色器。
    *   主界面与悬浮窗样式实时同步。

### 💄 UI/UX (体验优化)

*   **沉浸式设计**: 采用 Space Gray 深空灰背景与毛玻璃特效。
*   **品牌化**: 统一使用 "Eason Moment" 品牌落款。
*   **交互**: 优化了状态栏显示逻辑，避免文字过长；增加了导出时的仪式感动画。

### 🔧 Engineering (工程化)

*   集成 **GitHub Actions** 自动构建工作流 (macOS + Windows)。
*   配置 Prettier 代码格式化规范。
*   优化 Vite 构建配置，解决生产环境资源路径 (`file://`) 加载白屏问题。
